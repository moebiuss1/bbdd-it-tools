...
- **De dónde salen los datos nuevos**: `fetch_candidates.py` descubre jugadores con la **API de búsqueda de GitHub** (por `topic:`, nunca por texto libre: eso devolvía listas *awesome* y cursos) y el **landscape de la CNCF** (proyectos graduados y en incubación). Las fuentes anteriores —raspado de DuckDuckGo e itsm.tools— llevaban meses devolviendo cero y nadie lo veía porque el paso es `continue-on-error`. `scan_analyst_sources.py` recorre las webs de los fabricantes buscando los cuadrantes que publican: de ahí salen URLs reales de `gartner.com/reviews/market/<slug>` sin tocar Gartner, que responde 403 a todo, y también los mercados que el sector nombra y el catálogo aún no cubre. Ambos alimentan el aviso semanal de `check_catalog_health.py`.
- **El alta es editorial**: `enrich_tools.py` ya no crea fichas por su cuenta (`--allow-new` lo reactiva). Con el descubrimiento arreglado, cada lunes habría publicado hasta veinte fichas automáticas con texto de relleno —«X es una herramienta de Y», «Referente en el ámbito de Y»— a partir de lo que más estrellas tuviera en GitHub: servidores de correo, clientes HTTP y proyectos de la CNCF sin categoría. Decir que algo es un referente es un juicio, no un contador. Los candidatos se proponen en el aviso semanal y el alta se escribe a mano.
